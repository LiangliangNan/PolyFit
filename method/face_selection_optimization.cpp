...
void FaceSelection::optimize(PolyFitInfo* polyfit_info) {
	...
	BinaryProgram::Objective objective;
	...
	program_.set_objective(objective, total_variables);
	...
	for (std::size_t i = 0; i < fans.size(); ++i) {
		BinaryProgram::Constraint constraint;
		...
		constraint.set_fixed_bound(0.0);
	...
	for (std::size_t i = 0; i < fans.size(); ++i) {
		...
		BinaryProgram::Constraint edge_used_constraint;
		...
		edge_used_constraint.set_lower_bound(0.0);
		...
		for (std::size_t j = 0; j < fan.size(); ++j) {
			...
			for (std::size_t k = j + 1; k < fan.size(); ++k) {
				...
				if (plane1 != plane2) {
					...
					BinaryProgram::Constraint edge_sharp_constraint;
					...
					edge_sharp_constraint.set_lower_bound(1.0 - 3.0 * M);
	...
	Logger::out("-") << "formulating binary program done. Time: " << w.elapsed() << std::endl;
	...
	if (program_.solve(BinaryProgram::GUROBI)) {
		const std::vector<int>& X = program_.get_result();
		Logger::out("-") << "solving the binary program done. Time: " << w.elapsed() << std::endl;
