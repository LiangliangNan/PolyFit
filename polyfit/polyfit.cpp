...
namespace easy3d {
    ...
    namespace internal {
        ...
        void Candidate_confidences::compute(const PointCloud* point_set, SurfaceMesh* mesh) {
            ...
            const auto& points = point_set->get_vertex_property<vec3>("v:point").vector();
            float avg_spacing = compute_average_spacing(points, K);
            ...
            for(auto f : mesh->faces()) {
                const Plane3* supporting_plane = face_supporting_planes[f];
                // Face area
    ...
    SurfaceMesh* PolyFit::reconstruct(
        ...
        // The supporting plane of each face
        auto face_supporting_planes = target_mesh.add_face_property<const Plane3 *>("f:supp_plane");
