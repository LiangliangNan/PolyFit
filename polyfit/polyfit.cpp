...
namespace internal {
    ...
    public:
        ...
        Plane_3 *supporting_plane() const { return supporting_plane_; }

    private:
        const PointCloud *cloud_;
        Plane_3 *supporting_plane_; // The hypothesis generator owns this plane and manages the memory
    };
    ...
    Plane_3 *Planar_segment::fit_supporting_plane() {
        const auto& points = cloud_->points();
        std::list<Point_3> pts;
        for (std::size_t i = 0; i < size(); ++i) {
            std::size_t idx = at(i);
            const auto& p = points[idx];
            pts.push_back(Point_3(p.x, p.y, p.z));
        }

        if (supporting_plane_)
            delete supporting_plane_;
        supporting_plane_ = new Plane_3;
    ...
    private:
        ...
        std::size_t number_of_points_on_plane(const Planar_segment *s, const Plane_3 *plane, float dist_threshold);
        ...
        const vec3 *query_intersection(const Plane_3 *plane1, const Plane_3 *plane2, const Plane_3 *Plane_3);

        bool halfedge_exists(SurfaceMesh::Vertex v1, SurfaceMesh::Vertex v2, const SurfaceMesh &mesh);

        // Tests if 'face' insects 'plane'
        bool do_intersect(const SurfaceMesh &mesh, SurfaceMesh::Face face, const Plane_3 *plane);

        // Cuts face using the cutting_plane and returns the new faces
        std::vector<SurfaceMesh::Face> cut(SurfaceMesh::Face face, const Plane_3 *cutting_plane, SurfaceMesh &mesh);
        ...
        void compute_intersections(const SurfaceMesh &mesh,
                                   SurfaceMesh::Face face, const Plane_3 *cutting_plane,
        ...
        SurfaceMesh::Halfedge split_edge(SurfaceMesh &mesh, const EdgePos &ep, const Plane_3 *cutting_plane);
    ...
    private:
        ...
        std::vector<const Plane_3 *> supporting_planes_;

        // Precomputed intersecting points of all plane triplets
        std::vector<const vec3 *> intersecting_points_;

        typedef typename std::unordered_map<const Plane_3 *, const vec3 *> Plane_to_point_map;
        typedef typename std::unordered_map<const Plane_3 *, Plane_to_point_map> Two_planes_to_point_map;
        typedef typename std::unordered_map<const Plane_3 *, Two_planes_to_point_map> Planes_to_point_map;
    ...
    std::size_t
    Hypothesis::number_of_points_on_plane(const Planar_segment *s, const Plane_3 *plane, float dist_threshold) {
        ...
        for (std::size_t i = 0; i < s->size(); ++i) {
            ...
            const float sdist = CGAL::squared_distance(*plane, Point_3(p.x, p.y, p.z));
    ...
    void Hypothesis::merge(Planar_segment *s1, Planar_segment *s2) {
        ...
        if (pos != segments.end()) {
            ...
            const Plane_3 *plane = tmp->supporting_plane();
            segments.erase(pos);
            delete tmp;
            delete plane;
        } else
    ...
    void Hypothesis::refine_planes() {
        ...
        for (std::size_t i = 0; i < segments.size(); ++i) {
            ...
            const Plane_3 *plane = s->fit_supporting_plane(); // user may provide invalid plane fitting (we always fit)
        ...
        const float theta = static_cast<float>(M_PI * 10.0 / 180.0f);        // in radian
        bool merged = false;
        do {
            merged = false;
            ...
            std::sort(segments.begin(), segments.end(), SegmentSizeIncreasing<Planar_segment>());

            for (std::size_t i = 0; i < segments.size(); ++i) {
                Planar_segment *s1 = segments[i];
                const Plane_3 *plane1 = s1->supporting_plane();
                const Vector_3 n1 = plane1->orthogonal_vector();

                const float num_threshold = s1->size() / 5.0f;
                for (std::size_t j = i + 1; j < segments.size(); ++j) {
                    Planar_segment *s2 = segments[j];
                    const Plane_3 *plane2 = s2->supporting_plane();
                    const Vector_3 n2 = plane2->orthogonal_vector();

                    if (std::abs(n1 * n2) > std::cos(theta) * std::sqrt(n1.squared_length()) * std::sqrt(n2.squared_length())) {
        ...
        for (std::size_t i = 0; i < segments.size(); ++i) {
            Planar_segment *s = segments[i];
            const Plane_3 *plane = s->supporting_plane();
    ...
    SurfaceMesh Hypothesis::construct_bbox_mesh() {
        ...
        auto face_supporting_planes = mesh.add_face_property<const Plane_3 *>("f:supp_plane");

        // The supporting planes of each edge
        auto edge_supporting_planes = mesh.add_edge_property<std::set<const Plane_3 *> >("e:supp_plane");

        // The supporting planes of each vertex
        auto vertex_supporting_planes = mesh.add_vertex_property<std::set<const Plane_3 *> >("v:supp_plane");
        ...
        for (auto fd: mesh.faces()) {
            ...
            const Plane_3 *plane = new Plane_3(Point_3(pa.x, pa.y, pa.z), Point_3(pb.x, pb.y, pb.z), Point_3(pc.x, pc.y, pc.z));
            supporting_planes_.push_back(plane);
        ...
        for (auto ed: mesh.edges()) {
            ...
            const Plane_3 *plane1 = face_supporting_planes[f1];
            const Plane_3 *plane2 = face_supporting_planes[f2];
        ...
        for (auto vd: mesh.vertices()) {
            ...
            do {
                ...
                const Plane_3 *plane = face_supporting_planes[f];
    ...
    SurfaceMesh Hypothesis::construct_proxy_mesh(SurfaceMesh &bbox_mesh) {
        ...
        // The supporting planes of each edge
        auto bbox_edge_supporting_planes = bbox_mesh.add_edge_property<std::set<const Plane_3 *> >("e:supp_plane");

        // The supporting planes of each vertex
        auto bbox_vertex_supporting_planes = bbox_mesh.add_vertex_property<std::set<const Plane_3 *> >(
                "v:supp_plane");

        SurfaceMesh proxy_mesh;
        // The properties of the proxy mesh
        // The supporting plane of each face
        auto face_supporting_planes = proxy_mesh.add_face_property<const Plane_3 *>("f:supp_plane");
        ...
        // The supporting planes of each edge
        auto edge_supporting_planes = bbox_mesh.add_edge_property<std::set<const Plane_3 *> >("e:supp_plane");

        // The supporting planes of each vertex
        auto vertex_supporting_planes = bbox_mesh.add_vertex_property<std::set<const Plane_3 *> >("v:supp_plane");

        const std::vector<Planar_segment *> &segments = point_set_->planar_segments();
        for (std::size_t i = 0; i < segments.size(); ++i) {
            Planar_segment *g = segments[i];
            const Plane_3 *cutting_plane = g->supporting_plane();

            std::vector<vec3> intersecting_points;
            std::vector<std::set<const Plane_3 *> > intersecting_points_source_planes;
            ...
            for (auto ed: bbox_mesh.edges()) {
                ...
                const CGAL::Sign ss = cutting_plane->oriented_side(Point_3(s.x, s.y, s.z));
                const CGAL::Sign st = cutting_plane->oriented_side(Point_3(t.x, t.y, t.z));

                if ((ss == CGAL::POSITIVE && st == CGAL::NEGATIVE) || (ss == CGAL::NEGATIVE && st == CGAL::POSITIVE)) {
                    vec3 p;
                    if (cutting_plane->intersect(Line3::from_two_points(s, t))) {
                        intersecting_points.push_back(p);
                        std::set<const Plane_3 *> planes = bbox_edge_supporting_planes[ed];
                    ...
                    if (ss == 0 && st != 0) {
                        ...
                        const std::set<const Plane_3 *> &planes = bbox_vertex_supporting_planes[sd];
                        assert(planes.size() == 3);
                        intersecting_points_source_planes.push_back(planes);
                    } else if (st == 0 && ss != 0) {
                        intersecting_points.push_back(t);
                        const std::set<const Plane_3 *> &planes = bbox_vertex_supporting_planes[td];
                        assert(planes.size() == 3);
                        intersecting_points_source_planes.push_back(planes);
                    } else {
            ...
            if (intersecting_points.size() >= 3) {
                ...
                for (std::size_t i = 0; i < intersecting_points.size(); ++i) {
                    ...
                    pts.emplace_back(Point_3(q.x, q.y, float(i))); // the z component stores the point index
                }
                ...
                std::vector<vec3> ch;
                std::vector<std::set<const Plane_3 *> > ch_source_planes;
                ...
                if (ch.size() >= 3) {
                    ...
                    do {
                        ...
                        const std::set<const Plane_3 *> &s_planes = vertex_supporting_planes[s_vd];
                        const std::set<const Plane_3 *> &t_planes = vertex_supporting_planes[t_vd];
                        std::set<const Plane_3 *> common_planes;
    ...
    void Hypothesis::compute_triplet_intersections() {
        ...
        for (std::size_t i = 0; i < supporting_planes_.size(); ++i) {
            const Plane_3 *plane1 = supporting_planes_[i];
            for (std::size_t j = i + 1; j < supporting_planes_.size(); ++j) {
                const Plane_3 *plane2 = supporting_planes_[j];
                for (std::size_t k = j + 1; k < supporting_planes_.size(); ++k) {
                    const Plane_3 *Plane_3 = supporting_planes_[k];
                    assert(plane1 < plane2 && plane2 < Plane_3);
                    vec3 p;
                    if (intersection(*plane1, *plane2, *Plane_3, p)) {
                        // Stores the intersection for future query
                        vec3 *new_point = new vec3(p);
                        triplet_intersections_[plane1][plane2][Plane_3] = new_point;
    ...
    const vec3 *
    Hypothesis::query_intersection(const Plane_3 *min_plane, const Plane_3 *mid_plane, const Plane_3 *max_plane) {
    ...
    Hypothesis::split_edge(SurfaceMesh &mesh, const EdgePos &ep, const Plane_3 *cutting_plane) {
        // The supporting planes of each edge
        auto edge_supporting_planes = mesh.add_edge_property<std::set<const Plane_3 *> >("e:supp_plane");
        ...
        std::set<const Plane_3 *> sfs = edge_supporting_planes[ep.edge];
        assert(sfs.size() == 2);
    ...
    Hypothesis::cut(SurfaceMesh::Face face, const Plane_3 *cutting_plane, SurfaceMesh &mesh) {
        std::vector<SurfaceMesh::Face> new_faces;

        // The supporting plane of each face
        auto face_supporting_planes = mesh.face_property<const Plane_3 *>("f:supp_plane");
        const Plane_3 *supporting_plane = face_supporting_planes[face];
        ...
        // The supporting planes of each edge
        auto edge_supporting_planes = mesh.edge_property<std::set<const Plane_3 *> >("e:supp_plane");
    ...
    void Hypothesis::compute_intersections(const SurfaceMesh &mesh,
        ...
        // The supporting plane of each face
        auto face_supporting_planes = mesh.get_face_property<const Plane_3 *>("f:supp_plane");
        const Plane_3 *supporting_plane = face_supporting_planes[face];
        if (supporting_plane == cutting_plane)
            return;

        auto edge_supporting_planes = mesh.get_edge_property<std::set<const Plane_3 *> >("e:supp_plane");
        ...
        do {
            ...
            const std::set<const Plane_3 *> &supporting_planes = edge_supporting_planes[ed];
                       ...
                       (s_side == -1 && t_side == 1)) // intersects at the interior of the edge
            {
                ...
                if (s_sdist <= snap_squared_distance_threshold())                // plane cuts at vertex 's'
                    ...
                    const Plane_3 *plane1 = *(supporting_planes.begin());
                    const Plane_3 *plane2 = *(supporting_planes.rbegin());
                    const Plane_3 *Plane_3 = const_cast<const Plane_3 *>(cutting_plane);

                    if (Plane_3 != plane1 && Plane_3 != plane2) {
                        ::internal::sort_increasing(plane1, plane2, Plane_3);
                        const vec3 *p = query_intersection(plane1, plane2, Plane_3);
                        if (p) {
    ...
    bool Hypothesis::do_intersect(const SurfaceMesh &mesh, SurfaceMesh::Face f, const Plane_3 *plane) {
        std::vector<SurfaceMesh::Vertex> existing_vts;
        std::vector<EdgePos> new_vts;
    ...
    std::set<SurfaceMesh::Face> Hypothesis::collect_intersecting_faces(SurfaceMesh::Face face, const SurfaceMesh &mesh) {
        ...
        for (auto f: mesh.faces()) {
            ...
            const Plane_3 *plane = face_supporting_planes[face];
    ...
    void Hypothesis::pairwise_intersection(SurfaceMesh& mesh) {
        ...
        std::vector<SurfaceMesh::Face> all_faces;
        for (auto f : mesh.faces())
        ...
        for (std::size_t i = 0; i < all_faces.size(); ++i) {
            ...
            const Plane_3 *face_plane = face_supporting_planes[face];

            std::set<SurfaceMesh::Face> intersecting_faces = collect_intersecting_faces(face, mesh);
            ...
            while (!intersecting_faces.empty()) {
                ...
                const Plane_3 *cutting_plane = face_supporting_planes[cutting_face];
                ...
                for (std::size_t j = 0; j < faces_to_be_cut.size(); ++j) {
                    ...
                    std::vector<SurfaceMesh::Face> tmp = cut(current_face, cutting_plane, mesh);
            ...
            for (std::size_t j = 0; j < cutting_faces.size(); ++j) {
                cut(cutting_faces[j], face_plane, mesh);
            }
        }
    }
    ...
    Hypothesis::Adjacency Hypothesis::extract_adjacency(const SurfaceMesh &candidate_faces) {
        auto vertex_supporting_planes = candidate_faces.get_vertex_property<std::set<const Plane_3 *> >("v:supp_plane");
        ...
        for (auto h: candidate_faces.halfedges()) {
            ...
            const std::set<const Plane_3 *> &set_s = vertex_supporting_planes[sd];
            const std::set<const Plane_3 *> &set_t = vertex_supporting_planes[td];
            assert(set_s.size() == 3);
            assert(set_t.size() == 3);

            std::vector<const Plane_3 *> s_planes(set_s.begin(), set_s.end());
            ...
            std::vector<const Plane_3 *> t_planes(set_t.begin(), set_t.end());
    ...
    public:
        ...
        void compute(const PointCloud* cloud, SurfaceMesh* mesh);

    private:
        // Returns the indices of the supporting point for 'face'
        std::vector<std::size_t> supporting_points(SurfaceMesh::Face face, const SurfaceMesh* mesh, const PointCloud* point_set);

        inline float triangle_area(const vec3& p1, const vec3& p2, const vec3& p3) const {
            const vec3& orth = cross(p2 - p1, p3 - p1);
            return std::sqrt(orth.length2()) * 0.5f;
        }

        float face_area(SurfaceMesh::Face f, const SurfaceMesh* mesh) const {
            float result(0);

            SurfaceMesh::HalfedgeAroundFaceCirculator cir(mesh, f), done(cir);
            SurfaceMesh::Halfedge p_hd = *cir;
            SurfaceMesh::Vertex p_vd = mesh->target(p_hd);
            const vec3& p = mesh->position(p_vd);
            ++cir;

            do {
                SurfaceMesh::Halfedge q_hd = *cir;
                SurfaceMesh::Vertex q_vd = mesh->target(q_hd);
                const vec3& q = mesh->position(q_vd);

                SurfaceMesh::Halfedge r_hd = mesh->next(q_hd);
                SurfaceMesh::Vertex r_vd = mesh->target(r_hd);
                const vec3& r = mesh->position(r_vd);
        ...
        float average_spacing(const PointCloud *cloud, unsigned int k, bool accurate = false, unsigned int samples = 20000) {
    ...
    // implementation
    std::vector<std::size_t> Candidate_confidences::supporting_points(SurfaceMesh::Face face,
                                                                      const SurfaceMesh *mesh,
                                                                      const PointCloud *point_set) {
        ...
        const Plane_3* supporting_plane = face_supporting_planes[face];
        assert(supporting_plane == segment->supporting_plane());

        Polygon2 plg; // The projection of the face onto it supporting plane
        auto coords = mesh->get_vertex_property<vec3>("v:point");
        SurfaceMesh::HalfedgeAroundFaceCirculator cir(mesh, face), done(cir);
        do {
            SurfaceMesh::Halfedge hd = *cir;
            SurfaceMesh::Vertex vd = mesh->target(hd);
            const vec3& p = coords[vd];
            const vec2& q = supporting_plane->to_2d(p);

            // Removes duplicated vertices
            // The last point in the polygon
            if (!plg.empty()) {
                const vec2& r = plg[plg.size() - 1];
                if (distance2(q, r) < snap_squared_distance_threshold()) {
                    ++cir;
                    continue;
                }
            }
        ...
        const auto& points = point_set->get_vertex_property<vec3>("v:point").vector();
        for (std::size_t i = 0; i < segment->size(); ++i) {
            std::size_t idx = segment->at(i);
            const vec3& p = points[idx];
            if (plg.contains(supporting_plane->to_2d(p)))
    ...
    void Candidate_confidences::compute(const PointCloud* point_set, SurfaceMesh* mesh) {
        const unsigned int K = 6;

        const auto& points = point_set->get_vertex_property<vec3>("v:point").vector();
        float avg_spacing = average_spacing(point_set, K);

        // The number of supporting points of each face
        auto face_num_supporting_points = mesh->add_face_property<std::size_t>("f:num_supporting_points");

        // The area of each face
        auto face_areas = mesh->add_face_property<float>("f:face_area");

        // The point covered area of each face
        auto face_covered_areas = mesh->add_face_property<float>("f:covered_area");

        // The supporting plane of each face
        auto face_supporting_planes = mesh->get_face_property<const Plane_3*>("f:supp_plane");

        float degenerate_face_area_threshold = snap_squared_distance_threshold() * snap_squared_distance_threshold();

        for(auto f : mesh->faces()) {
            const Plane_3* supporting_plane = face_supporting_planes[f];
            // Face area
            const float area = face_area(f, mesh);
            face_areas[f] = area;
            ...
            if (area > degenerate_face_area_threshold) {
                ...
                ::internal::Alpha_shape_mesh alpha_mesh(pts.begin(), pts.end(), *supporting_plane);
                SurfaceMesh covering_mesh;
                ...
                if (alpha_mesh.extract_mesh(radius * radius, covering_mesh)) {
                    ...
                    auto coords = covering_mesh.get_vertex_property<vec3>("v:point");
                    for(auto face : covering_mesh.faces()) {
                        ...
                        SurfaceMesh::HalfedgeAroundFaceCirculator cir(&covering_mesh, face), done(cir);
                        do {
                            SurfaceMesh::Halfedge hd = *cir;
                            SurfaceMesh::Vertex vd = covering_mesh.target(hd);
                            const vec3& p = coords[vd];
                            const vec2& q = supporting_plane->to_2d(p);
                            plg.push_back(q);
                            ++cir;
            ...
            else { // For tiny faces, we can simply assign zero supporting points
                face_num_supporting_points[f] = 0;
                face_covered_areas[f] = 0.0f;
            }
        }
    }
}
...
SurfaceMesh* PolyFit::reconstruct(
    ...
    if (candidate_faces_->n_faces() < 4) {
        error_message_ = "at least 4 candidate faces required to reconstruct a closed surface mesh (only "
                         + std::to_string(candidate_faces_->n_faces()) + " computed)";
        return nullptr;
    }

    typedef typename internal::Hypothesis::Adjacency Adjacency;
    ...
    // The number of supporting points of each face
    auto face_num_supporting_points = target_mesh.add_face_property<std::size_t>("f:num_supporting_points");

    // The area of each face
    auto face_areas = target_mesh.add_face_property<float>("f:face_area");

    // The point covered area of each face
    auto face_covered_areas = target_mesh.add_face_property<float>("f:covered_area");

    // The supporting plane of each face
    auto face_supporting_planes = target_mesh.add_face_property<const Plane_3 *>("f:supp_plane");

    // Gives each face an index
    auto face_indices = target_mesh.add_face_property<std::size_t>("f:index");
    ...
    std::size_t num_faces = target_mesh.n_faces();
    std::size_t num_edges(0);

    typedef typename internal::Hypothesis::Intersection Intersection;
    ...
    const Box3 box = target_mesh.bounding_box();
    const float box_area = box.surface_area();
    ...
    if (solver.solve()) {
        ...
        for (std::size_t i = 0; i < to_delete.size(); ++i) {
            ...
            target_mesh.delete_face(f);
        }

        // Marks the sharp edges
        auto edge_is_sharp = target_mesh.add_edge_property<bool>("e:sharp_edges");
        ...
        for (std::size_t i = 0; i < adjacency.size(); ++i) {
            ...
            if (static_cast<int>(X[idx_sharp_var]) == 1) {
                for (std::size_t j = 0; j < fan.size(); ++j) {
                    ...
                    if (f.is_valid()) { // some faces may be deleted
    ...
    return new SurfaceMesh(target_mesh);
}
