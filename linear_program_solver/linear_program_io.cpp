...
namespace details {
	...
	std::vector<std::pair<const Variable*, double>> ordered_coefficients(const LinearExpression* expr, bool increasing_order /* = true*/) {
		std::vector<std::pair<const Variable*, double>> coeffs(expr->coefficients().begin(), expr->coefficients().end());
		...
		if (increasing_order) {
			struct SortIncreasing {
				bool operator()(const std::pair<const Variable*, double>& a, const std::pair<const Variable*, double>& b) {
...
#ifdef SORT_VARIABLES_IN_INCREASING_ORDER
		const std::vector< std::pair<const Variable*, double> >& coeffs = details::ordered_coefficients(c, true);
		std::vector< std::pair<const Variable*, double> >::const_iterator cur = coeffs.begin();
#else
...
#ifdef SORT_VARIABLES_IN_INCREASING_ORDER
	const std::vector< std::pair<const Variable*, double> >& obj_coeffs = details::ordered_coefficients(objective_, true);
	std::vector< std::pair<const Variable*, double> >::const_iterator cur = obj_coeffs.begin();
#else
...
bool LinearProgram::load(const std::string& file_name) {
	...
	for (std::size_t i = 0; i < num_cons; ++i) {
		...
		for (std::size_t j = 0; j < nv; ++j) {
			...
			c->add_coefficient(variables[idx], coeff);
		}
	...
	for (std::size_t i = 0; i < num_var; ++i) {
		...
		objective_->add_coefficient(variables[idx], coeff);
	}
