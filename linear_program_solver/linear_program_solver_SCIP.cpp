...
#include <linear_program_solver/linear_program_solver_SCIP.h>
...
bool LinearProgramSolver_SCIP::solve(const LinearProgram* program) {
	try {
		if (!program->is_valid())
			return false;
...
//		SCIP_CALL(SCIPfreeTransform(scip));
		...
		// Time limit.
		int64_t time_limit = 0; // change if you don't want to wait after certain time
		if (time_limit != 0) {
			std::cerr << "setting time limit = " << time_limit << " seconds.";
		...
		if (SCIPsolve(scip) == SCIP_OKAY) {
			...
			if (sol) {
				...
				objective_value_ = SCIPgetSolOrigObj(scip, sol) + objective->offset();
