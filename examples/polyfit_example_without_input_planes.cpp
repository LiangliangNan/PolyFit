...
int main(int, char**)
{
    const std::string input_file = "/Users/lnan/Documents/Projects/PolyFit/toy_data.bvg";
    const std::string output_file = "/Users/lnan/Documents/Projects/PolyFit/toy_data-result.obj";

    auto cloud = PointCloudIO::load(input_file);
    if (!cloud) {
        std::cerr << "failed to load point cloud from file: " << input_file << std::endl;
        return EXIT_FAILURE;
    }

    PolyFit polyfit(cloud, cloud->vertex_property<int>("v:primitive_index"));
    auto mesh = polyfit.reconstruct();
    if (!mesh) {
        std::cerr << "failed in reconstruction: " << polyfit.error_message() << std::endl;
        return EXIT_FAILURE;
    }

    if (SurfaceMeshIO::save(output_file, mesh)) {
        std::cerr << "reconstruction result saved to: " << output_file << std::endl;
        return EXIT_SUCCESS;
    } else {
        std::cerr << "failed to save reconstruction result to file: " << polyfit.error_message() << std::endl;
        return EXIT_FAILURE;
    }
}
